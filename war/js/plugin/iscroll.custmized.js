...
(function() {
	...
	has3d = 'WebKitCSSMatrix' in window && 'm11' in new WebKitCSSMatrix(), hasTouch = 'ontouchstart' in window, hasTransform = vendor
			+ 'Transform' in document.documentElement.style, isAndroid = (/android/gi)
			.test(navigator.appVersion), isIDevice = (/iphone|ipad/gi)
			.test(navigator.appVersion), isPlaybook = (/playbook/gi)
			.test(navigator.appVersion), hasTransitionEnd = isIDevice
	...
	iScroll = function(el, options) {
		...
		if (that.options.useTransform)
			that.scroller.style[vendor + 'Transform'] = trnOpen + '0,0'
					+ trnClose;
		else
	...
	iScroll.prototype = {
		...
		_move : function(e) {
			...
			if (newY > 0 || newY < that.maxScrollY) {
				newY = that.options.bounce ? that.y + (deltaY / 2) : newY >= 0
		...
		_end : function(e) {
			...
			var that = this, point = hasTouch ? e.changedTouches[0] : e, target, ev, momentumX = {
				...
				dist : 0,
				time : 0
			}, duration = (e.timeStamp || (new Date()).getTime())
					- that.startTime, newPosX = that.x, newPosY = that.y, distX, distY, newDuration, scale;
		...
		_resetPos : function(time) {
			var that = this, resetX = that.x >= 0 ? 0
					: that.x < that.maxScrollX ? that.maxScrollX : that.x, resetY = that.y >= 0
					|| that.maxScrollY > 0 ? 0
		...
		_wheel : function(e) {
			var that = this, duration, wheelDeltaX, wheelDeltaY, deltaX, deltaY, deltaScale;
			...
			if (that.options.wheelAction == 'zoom') {
				...
				if (wheelDeltaY < that.options.zoomMin)
					wheelDeltaY = that.options.zoomMin;
				if (wheelDeltaY > that.options.zoomMax)
					wheelDeltaY = that.options.zoomMax;
		...
		disable : function() {
			...
			// If disabled after touchstart we make sure that there are no left
			// over events
