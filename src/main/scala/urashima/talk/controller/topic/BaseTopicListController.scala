...
trait BaseTopicListController extends AbstractActionController {
  override def replacerMap: Map[String, ((Node) => NodeSeq)] = {
    ...
    { e =>  <a id="menuAdd" class="center" href="/topic/form" data-rel="dialog" data-transition="slidedown"><img id="menuAddIcon" src="/img/icon/add.png" alt={"%s %s".format(LanguageUtil.get("topic"), LanguageUtil.get("add"))} height="16" style="margin:0px;" /></a> })
  }
}